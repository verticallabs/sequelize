...
module.exports = (function() {
  ...
  */
  var groupDataByCalleeFactory = function(data) {
    var result          = [];
    var calleeTableName = this.callee.tableName;

    data.forEach(function(row) {
      var calleeData    = row[calleeTableName]
      var existingEntry = result.filter(function(groupedRow) {
      ...
      for (var attrName in row) {
        if (row.hasOwnProperty(attrName) && (attrName !== calleeTableName)) {
          ...
          var alreadyExists = existingEntry[attrName].filter(function(groupedRow) { 
            return Utils._.isEqual(groupedRow, row[attrName]) 
          })[0];
